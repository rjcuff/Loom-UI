...
stitches rather than a line. A dashed path cannot be wiped with its own dash offset, because both want the
same property. So the reveal is a second copy of the path, stroked solid and
wiped, used as a mask.
...
By default it follows its own box through the viewport. Nothing when its top
edge sits on the bottom of the screen, fully sewn once its bottom edge clears
the top.
