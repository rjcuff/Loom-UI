...
which is what hides the seam in the loop. The whole thing is `transform` on a
...
gradient. A linear alpha ramp meets full opacity at a point the eye reads as a
hard line.
...
<Callout type="tip" title="Accessibility">
  Only the first copy of each column is read. The duplicate that hides the seam
  is `aria-hidden`, so no quote is announced twice. `pauseOnHover` is on by
  default, which is what makes a quote readable at all without chasing it.
</Callout>
