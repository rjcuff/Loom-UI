...
left. It runs back out eased and on its own quick clock, so letting go early
reads as a slip you can recover from rather than a state being dropped. A
completed hold sits at full for a beat first, so the action has a visible end.
