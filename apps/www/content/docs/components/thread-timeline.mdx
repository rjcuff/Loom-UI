...
drawn to the same place. Because both read the same number, a node can never
light ahead of the thread that is meant to reach it, which is the thing that
gives a timeline built out of separate observers away.
...
marked reached on mount, so nothing is left greyed out waiting for a scroll
that will not animate.
